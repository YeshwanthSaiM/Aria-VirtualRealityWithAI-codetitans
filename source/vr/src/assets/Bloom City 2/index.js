...
AFRAME.registerComponent('hotspots', {
  init: async function () {
    const jsonURL = "assets/Bloom City 2/map.json";
    const hotspotsEl = this.el;

    map_ = await fetchMap(jsonURL);
    const reloadedHotspots = await addReloadSpotsEvent(hotspotsEl, map_);
    const hotSpots = await addHotspots(map_, reloadedHotspots);
  }
});
...
window.addEventListener("wheel", event => {
  ...
  var cam = document.getElementById('cam').getAttribute('camera');
  ...
  document.getElementById('cam').setAttribute('camera', cam);
});
...
function addSpot(spotAImage, navData) {
  ...
  box.setAttribute("position", "0 1.6 0");
  box.setAttribute("opacity", "0");
  spotAImage.appendChild(box);
  spotAImage.addEventListener('mouseenter', (e) => {
    box.setAttribute("src", navData.linkto); // This gives the item or value stored in scenes from nav which is in spots to the eventListner()
    console.log(navData.linkto); //This gives the item or value stored in scenes from nav which is in spots in console window
    spotAImage.children[0].setAttribute("opacity", "1");
  })
  spotAImage.addEventListener('mouseleave', (e) => {
    spotAImage.children[0].setAttribute("opacity", "0");
  })
 ...
 * @param {object} map_ - dependency map between scenes, from external .json file
 */
function addReloadSpotsEvent(hotspotsEl, map_) {
 ...
 * @param {string} jsonURL - path to file: /map.json
 */
 ...
 */
function addHotspots(map_, reloadedHotspots) {
  ...
  map_.spots.forEach(spot => {
    ...
    spot.nav.forEach(nav => {
      ...
      const scene = map_.scenes.filter(scene => (
        scene.id === spotScene
      ))[0];
      ...
      // Used For positioning of hotspot
      newImage.setAttribute('position', nav.position);
      // Used For inclination and orientation of hotspot
      newImage.setAttribute('rotation', nav.inclination);
